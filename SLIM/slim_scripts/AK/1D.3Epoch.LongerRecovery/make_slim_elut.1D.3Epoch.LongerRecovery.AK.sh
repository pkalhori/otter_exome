...
//calculate load every 2 generations
$((${t} + 1)):$((${t} + ${tcontract})) late() {
	if (sim.generation % 2 == 0){
		...
		//these are genotype counts not allele counts
		// set up outfile: 
		writeFile(paste(c(outdir,"/slim.output.Contraction.",sim.generation,"gen.",v_CHUNK,".summary.txt"),sep=""),"replicate,chunk,mutid,type,s,age,subpop,p1numhet,p1numhom,popsizeDIP\n",append=F); // open fresh file
		...
		for (mut in sim.mutations){
			...
			//print results
			writeFile(paste(c(outdir,"/slim.output.Contraction.",sim.generation,"gen.",v_CHUNK,".summary.txt"),sep=""),paste(c(v_REP,v_CHUNK,id,type,s,age,subpop,p1numhet,p1numhom,popsize),sep=","),append=T);
		}
	}
}
...
$((${t} + 2+ ${tcontract})):$((${t} + 1+ ${tcontract}+${trecovery})) late() {
	if (sim.generation % 2 == 0){
		...
		//these are genotype counts not allele counts
		// set up outfile: 
		writeFile(paste(c(outdir,"/slim.output.Recovery.",sim.generation,"gen.",v_CHUNK,".summary.txt"),sep=""),"replicate,chunk,mutid,type,s,age,subpop,p1numhet,p1numhom,popsizeDIP\n",append=F); // open fresh file
		...
		for (mut in sim.mutations){
			...
			//print results
			writeFile(paste(c(outdir,"/slim.output.Recovery.",sim.generation,"gen.",v_CHUNK,".summary.txt"),sep=""),paste(c(v_REP,v_CHUNK,id,type,s,age,subpop,p1numhet,p1numhom,popsize),sep=","),append=T);
		}
	}
}
...
$((${t} + 3+ ${tcontract}+${trecovery})):$((${t} + 3+ ${tcontract}+${trecovery}+${tfuture})) late() {
	if (sim.generation % 2 == 0){
		...
		//these are genotype counts not allele counts
		// set up outfile: 
		writeFile(paste(c(outdir,"/slim.output.Future.",sim.generation,"gen.",v_CHUNK,".summary.txt"),sep=""),"replicate,chunk,mutid,type,s,age,subpop,p1numhet,p1numhom,popsizeDIP\n",append=F); // open fresh file
		...
		for (mut in sim.mutations){
			...
			//print results
			writeFile(paste(c(outdir,"/slim.output.Future.",sim.generation,"gen.",v_CHUNK,".summary.txt"),sep=""),paste(c(v_REP,v_CHUNK,id,type,s,age,subpop,p1numhet,p1numhom,popsize),sep=","),append=T);
		}
	}
}
