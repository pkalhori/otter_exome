...
popModDates=c("CA_AK/2D.3Epoch.Translocation/20191006")
...
reps=1 # some reps don't make it through Hoffman; so I have a file.exists() test in the loop to skip reps that didn't yield output
hset=c(0,0.5)
...
write.table(allLoads, file="load.test.txt",quote = F, row.names = F, sep="\t")
