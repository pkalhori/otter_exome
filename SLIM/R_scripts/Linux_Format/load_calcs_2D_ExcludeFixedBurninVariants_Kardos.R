...
outdir="/u/scratch/p/pkalhori/slim/R_load_calc/CA_AK/"
...
popModDates=c("CA_AK/2D.3Epoch.NoTranslocation/20210927/", "CA_AK/2D.3Epoch.Translocation.1perGen/20210927/","CA_AK/2D.3Epoch.Translocation.25perGen/20210927/", "CA_AK/2D.3Epoch.Translocation.25for2Gen/20210927/" ) # AK and AL have dadi parameters, genericPop has parameters based on AK MLE grid that is fur-trade relevant. ### need to come up with better classification system for this. 
#reps=c(seq(1,23))
  ...
  for(rep in reps){
    ...
    if(file.exists(infile)){
      ...
      avgHomPerIndPersCat <- inputWithFixedRemoved %>%
        group_by(generation,htype,population,subpop,sCat,model,replicate,popModDate,popsizeDIP) %>%
      ...
      LoadPerGeneration <- inputWithFixedRemoved %>%
        group_by(generation,htype,population,subpop,model,replicate,popModDate,popsizeDIP) %>%
