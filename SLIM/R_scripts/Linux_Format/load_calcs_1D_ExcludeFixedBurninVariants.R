...
for(popModDate in popModDates){
  for(rep in reps){
    ...
    infile=paste(data.dir,popModDate,htype,"/replicate_",rep,".slim.output.allConcatted.summary.txt.gz",sep="")
      if(file.exists(infile)){
        ...
        LoadPerGeneration <- inputWithFixedRemoved %>%
          group_by(generation,population,model,replicate,popModDate,htype,popsizeDIP) %>%
...
write.table(allAvgdInputs,paste(outdir,todaysdate,"_AvgHomozygousDerivedGTs.PerInd.ThroughTime.AllReps.RemovedBurninFixedVar.txt",sep=""),row.names = F,col.names = T,quote=F,sep="\t")
write.table(allLoads,paste(outdir,todaysdate,"_LoadPerGeneration.ThroughTime.AllReps.RemovedBurninFixedVar.txt",sep=""),row.names = F,col.names = T,quote=F,sep="\t")
