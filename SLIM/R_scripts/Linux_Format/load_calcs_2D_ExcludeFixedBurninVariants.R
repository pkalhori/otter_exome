...
popModDates=c("CA_AK/2D.3Epoch.NoTranslocation/20200226/", ) # AK and AL have dadi parameters, genericPop has parameters based on AK MLE grid that is fur-trade relevant. ### need to come up with better classification system for this. 
#reps=c(seq(1,23))
...
for(h in hset){
  ...
  if(file.exists(infile)){
    ...
    avgHomPerIndPersCat <- inputWithFixedRemoved %>%
      group_by(generation,h,population,subpop,sCat,model,replicate,popModDate,popsizeDIP) %>%
    ...
    LoadPerGeneration <- inputWithFixedRemoved %>%
      group_by(generation,h,population,subpop,model,replicate,popModDate,popsizeDIP) %>%
